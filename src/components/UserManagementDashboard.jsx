...
import { supabaseAdmin, isAdminAvailable, getAdminErrorMessage } from '../supabaseAdmin';
...
const UserManagementDashboard = () => {
  ...
  const fetchUsersManually = async () => {
    try {
      ...
      const [usersResponse, userRolesResponse, rolesResponse] = await Promise.all([
        supabase.from('a_users').select('*').order('created_at', { ascending: false }),
        supabase.from('a_user_roles').select('user_id, role_id'),
        supabase.from('a_roles').select('id, name, description')
      ]);
      ...
      const usersWithRoles = usersData.map(user => {
        const userRoles = userRolesData
          ...
          .map(ur => {
            ...
            return {
              role_id: ur.role_id,
              a_roles: role || { id: ur.role_id, name: 'Unknown Role', description: '' }
            };
          });

        return {
          ...user,
          a_user_roles: userRoles
        };
      });
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      // Insert user into a_users table (include password_hash for compatibility)
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      // Delete from a_user_roles first (due to foreign key constraint)
      await supabase
        .from('a_user_roles')
        .delete()
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(user => {
      ...
      const matchesRole = !filterRole || user.a_user_roles?.some(ur => ur.role_id.toString() === filterRole);
      return matchesSearch && matchesRole;
    });
  }, [users, searchTerm, filterRole]);

  const getUserRoles = useCallback((user) => {
    return user.a_user_roles?.map(ur => ur.a_roles?.name).filter(Boolean) || [];
  }, []);

  const hasRole = useCallback((user, roleId) => {
    return user.a_user_roles?.some(ur => ur.role_id === roleId) || false;
  }, []);
