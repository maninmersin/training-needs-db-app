...
const UserPool = ({
  ...
  const getUniqueTrainingLocations = () => {
    ...
    [...(userCategories.allCoursesNeeded || []), 
     ...Object.values(userCategories.someCoursesNeeded || {}).flat(),
     ...(userCategories.unassigned || [])]
      .forEach(user => {
        if (user.training_location) {
          locations.add(user.training_location);
        }
      });
...
const DraggableUser = ({ 
  ...
  return (
    <div
      ...
      onClick={handleClick}
      className={`user-item ${dragMode ? 'draggable' : ''} ${isDragging ? 'dragging' : ''} ${isSelected ? 'selected' : ''}`}
    >
