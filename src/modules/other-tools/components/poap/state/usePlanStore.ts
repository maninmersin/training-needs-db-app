    ...
    persist(
      (set, get) => ({
        ...
        createPlan: async (title: string, timelineConfig?: Partial<TimelineConfig>) => {
          ...
          try {
            ...
            if (sessionError || !session?.user?.id) {
              throw new Error('Authentication required: Please log in to create plans');
            }
            ...
            newPlan.ownerId = session.user.id;
            ...
            try {
              ...
              const updatedPlans = get().deduplicatePlans([...get().plans.filter(p => p.id !== planWithId.id), planWithId]);
            ...
            const plan: Plan = {
              ...
              swimlanes: (swimlanesData || []).map(swimlane => ({
                ...
                milestone: swimlane.milestone_title ? {
                  ...
                  date: new Date(swimlane.milestone_date),
              ...
              cards: (cardsData || []).map(card => ({
                ...
                milestone: card.milestone_title ? {
                  ...
                  date: new Date(card.milestone_date),
                  color: card.milestone_color
                } : undefined
              })),
              milestones: [], // Milestones are now embedded in swimlanes and cards
              shapes: (shapeElementsData || []).map(shapeElement => ({
                id: shapeElement.id,
                planId: shapeElement.plan_id,
                shapeType: shapeElement.shape_type as ShapeType,
                position: {
                ...
                order: shapeElement.properties?.order || 0,
                createdAt: new Date(shapeElement.created_at),
                updatedAt: new Date(shapeElement.updated_at)
              })),
              texts: (textElementsData || []).map(textElement => ({
                ...
                style: {
                  ...
                  textAlign: textElement.text_align as 'left' | 'center' | 'right'
                },
                ...
                updatedAt: new Date(textElement.updated_at)
              }))
            };
        ...
        saveAsNewPlan: async (newName: string, newTitle: string) => {
          const { currentPlan, isLoading } = get();
          
          if (!currentPlan) {
            console.log('❌ No current plan found for Save As');
            return;
          }
          
          if (isLoading) {
            console.log('⚠️ Save As operation already in progress, ignoring duplicate call');
            return;
          }
          
          const operationId = Math.random().toString(36).substring(2, 8);
          console.log(`🔄 [${operationId}] Starting Save As for plan:`, currentPlan.name);
          console.log(`📝 [${operationId}] Save As parameters:`, { newName, newTitle });
          console.log('📊 Current plan has:', {
            cards: currentPlan.cards.length,
            swimlanes: currentPlan.swimlanes.length,
            texts: currentPlan.texts.length,
            shapes: currentPlan.shapes.length
          });
          
          set({ isLoading: true, error: null });
          
          try {
            // Create a copy of the current plan with new ID, name, and title
            const newPlan: Plan = {
              ...currentPlan,
              id: generateId(),
              name: newName,
              ...
              swimlanes: currentPlan.swimlanes.map(swimlane => ({
                ...
                parentId: swimlane.parentId // Will need to remap parent relationships
              })),
              cards: currentPlan.cards.map(card => ({
                ...card,
                id: generateId(),
                planId: '', // Will be set after plan creation
                swimlaneId: '' // Will need to remap swimlane relationships
              })),
            ...
            currentPlan.swimlanes.forEach((oldSwimlane, index) => {
              swimlaneIdMap.set(oldSwimlane.id, newPlan.swimlanes[index].id);
            });
            
            // Fix parent relationships in swimlanes
            newPlan.swimlanes = newPlan.swimlanes.map(swimlane => ({
              ...swimlane,
              planId: newPlan.id,
              parentId: swimlane.parentId ? (swimlaneIdMap.get(swimlane.parentId) || null) : null
            }));
            
            // Fix swimlane relationships in cards
            newPlan.cards = newPlan.cards.map(card => ({
              ...card,
              planId: newPlan.id,
              swimlaneId: swimlaneIdMap.get(card.swimlaneId) || newPlan.swimlanes[0]?.id || ''
            }));
            
            // Fix plan IDs in texts and shapes
            newPlan.texts = newPlan.texts.map(text => ({
              ...text,
              planId: newPlan.id
            }));
            
            newPlan.shapes = newPlan.shapes.map(shape => ({
              ...shape,
              planId: newPlan.id
            }));
            
            // Ensure the new plan name is unique before saving
            const finalUniqueName = await get().generateUniquePlanName(newPlan.name);
            newPlan.name = finalUniqueName;

            // Get current authenticated user
            const { data: { session }, error: sessionError } = await supabase.auth.getSession();
            
            if (sessionError || !session?.user?.id) {
              throw new Error('Authentication required: Please log in to save plans');
            }

            // Save the new plan to database (reuse createPlan logic but with our custom plan)
            console.log(`💾 [${operationId}] Inserting plan to database:`, { id: newPlan.id, name: newPlan.name, title: newPlan.title });
            const { data: planData, error: planError } = await supabase
              .from('plans')
              .insert({
                id: newPlan.id,
            ...
            if (planError) {
              ...
              if (planError.message?.includes('unique') || planError.message?.includes('duplicate')) {
                throw new Error('A plan with this name already exists. Please try again.');
              }
              throw planError;
            }
            ...
            // Save all elements to database (similar to savePlan but with insert)
            if (planWithId.swimlanes.length > 0) {
              const { error: swimlanesError } = await supabase
                .from('swimlanes')
                .insert(planWithId.swimlanes.map(swimlane => ({
            ...
            if (planWithId.cards.length > 0) {
              const { error: cardsError } = await supabase
                .from('cards')
                .insert(planWithId.cards.map(card => ({
                  id: card.id,
                  plan_id: card.planId,
                  swimlane_id: card.swimlaneId,
                  title: card.title,
              ...
              if (cardsError) throw cardsError;
              console.log(`✅ [${operationId}] Successfully saved`, planWithId.cards.length, 'cards to database');
            } else {
              console.log('ℹ️ No cards to save');
            }
            
            if (planWithId.texts.length > 0) {
              const { error: textElementsError } = await supabase
                .from('text_elements')
                .insert(planWithId.texts.map(textElement => ({
                  id: textElement.id,
                  plan_id: textElement.planId,
                  content: textElement.content,
                  position_x: textElement.position.x,
                  ...
                  background_color: textElement.style.backgroundColor,
                  text_align: textElement.style.textAlign,
                  is_selected: false, // Reset selection state
            ...
            if (planWithId.shapes.length > 0) {
              const { error: shapeElementsError } = await supabase
                .from('shape_elements')
                .insert(planWithId.shapes.map(shapeElement => ({
                  id: shapeElement.id,
                  plan_id: shapeElement.planId,
                  shape_type: shapeElement.shapeType,
                  position_x: shapeElement.position.x,
            ...
            const cleanPlan: Plan = {
              ...planWithId,
              texts: planWithId.texts.map(text => ({
                ...text,
                isSelected: false,
                isEditing: false
              })),
              shapes: planWithId.shapes.map(shape => ({
                ...shape,
                isSelected: false
              }))
            };
            ...
            const planHash = createPlanHash(cleanPlan);
            console.log(`✅ [${operationId}] Save As completed successfully!`);
            console.log(`📋 [${operationId}] New plan created:`, {
              name: cleanPlan.name,
              title: cleanPlan.title,
              cards: cleanPlan.cards.length,
              swimlanes: cleanPlan.swimlanes.length,
              texts: cleanPlan.texts.length,
              shapes: cleanPlan.shapes.length
            });
            
            set({ 
              currentPlan: cleanPlan,
              isLoading: false,
              lastSavedHash: planHash,
              hasUnsavedChanges: false
            });
            ...
            // Refresh the plans list from database to ensure consistency
            console.log(`🔄 [${operationId}] Refreshing plans list from database`);
            try {
              await get().loadPlans();
              console.log(`✅ [${operationId}] Plans list refreshed successfully`);
            } catch (refreshError) {
              console.warn(`⚠️ [${operationId}] Failed to refresh plans list:`, refreshError);
              // Fallback: manually add the plan if refresh fails
              const updatedPlans = get().deduplicatePlans([...get().plans.filter(p => p.id !== cleanPlan.id), cleanPlan]);
              set({ plans: updatedPlans });
            }
        ...
        // Helper function to ensure no duplicate plans in store
        ...
        loadPlans: async () => {
          ...
          try {
            ...
            // Ensure no duplicates in the plans array
            const deduplicatedPlans = get().deduplicatePlans(plans);
        ...
        moveCard: (id: string, swimlaneId: string, startDate: Date, endDate: Date) => {
          ...
          // Clamp start date to timeline bounds
          const clampedStartDate = new Date(Math.max(
            timelineStart.getTime(),
            Math.min(startDate.getTime(), timelineEnd.getTime() - 24 * 60 * 60 * 1000) // Ensure at least 1 day before timeline end
          ));
          
          // Clamp end date to timeline bounds
          const clampedEndDate = new Date(Math.max(
            clampedStartDate.getTime() + 24 * 60 * 60 * 1000, // Ensure at least 1 day after start
        ...
        addMilestone: (milestone: Omit<Milestone, 'id'>) => {
          // Convert to embedded milestone in swimlane or card
          const { currentPlan } = get();
          if (!currentPlan) return;
          
          // This is a legacy method - milestones are now embedded
