 ...
 * @param {string} projectId - The project ID for data filtering
 * @returns {Promise<Object>} - {success, filename, content, error}
 */
export const generateTrainingCalendar = async (schedule, sessions, assignments, projectId) => {
  if (!projectId) {
    throw new Error('Project ID is required for calendar generation');
  }
  try {
    ...
    const usersWithEmails = await getUsersWithEmails(assignments, projectId);
 ...
 */
const getUsersWithEmails = async (assignments, projectId) => {
  if (!projectId) {
    throw new Error('Project ID is required to fetch user details');
  }
  try {
    ...
    // Fetch user details including email addresses (with project isolation)
